...
<%
	...
	int loginId = 4;  //(Integer)session.getAttribute("loginJH");
...
/*             background-image: url(/COLLABEE/img/emiji.png); *//*이모티콘*/
    ...
    $(function(){
    	...
    	/* $.ajax({ 
			type:"post",
			url:"Controller?command=select_Workspace_Index",
			data:{
				"loginId":4,
    	...
    	$.ajax({
			...
			success:function(data){
				...
				if(data.length==0){
					...
					$.each(data, function(index, item) {
	         			var box = "<a href='postedDocument.jsp?documentId="+item.document_id+"&workspaceId="+workspaceId+"&workspaceName="+workspaceName+"'>"
	         			+ "<div class='post_wi'>"
        ...
        $("#alarm_wi").click(function(){
            ...
            //var workspace_id = workspaceId//$("#title_wi").attr("workspaceid");
        ...
        $("#title_wi").click(function(){
        	location.href="workspace_index.jsp?workspaceId="+workspaceId+"&workspace_name="+workspaceName;
        });
...
/* 
    ...
    <script>
	$(document).on("click", "#document_bnt_wi", function(){ //문서작성버튼
		$.ajax({
			type:"post",
			...
			datatype:"json",
			success:function(data){
				location.href="Document.jsp?documentId="+data.document_id+"&workspaceId="+workspaceId+"&workspaceName="+workspaceName;
			},
			error:function(data){
				alert("error!");
			}
...
<body>
	...
	<%@ include file="../WEB-INF/include/include_InviteMembers.jsp"%>
    ...
    <section id="space_main_wi"> <!--제이쿼리, 왼쪽상단, 하단, 필터 선택&열고닫기 -->
        <header>
            ...
            <div class="topright3 topright1">
                <button type="button" title="유라짱짱" class="btn11"></button>
                <button type="button" title="파트너 초대" class="btn21_wi">
                ...
                <button type="button" class="btn31_wi">
                    <span>멤버</span>
        ...
        <div id="tab_wi">
            <div id="container1_wi"> <!-- 협업공간탭 -->
                ...
                <button type="button"><span>내가 작성한 문서</span></button>
            </div>
